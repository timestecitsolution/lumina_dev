                ...
                <div class="row p-20">
                    ...
                    @if (user()->permission('add_expenses') == 'all')
                        <div class="col-md-6 col-lg-4">
                            ...
                            <x-forms.label class="my-3" fieldId="selectUser"
                                :fieldLabel="__('app.employee')"
                            />

                            <x-forms.input-group>
                                <select class="form-control multiple-user" multiple name="user_id[]" id="selectUser" data-live-search="true"
                                    data-size="8">
                                    @foreach ($employees as $item)
                                        <option
                                            value="{{ $item->id }}"
                                            @if(in_array($item->id, $selectedEmployee)) selected @endif
                                        >
                                            {{  $item->name }}
                                        </option>
                                    @endforeach
                    ...
                    <div class="col-md-6 col-lg-4">
                        ...
                        <x-forms.input-group>
                            <select class="form-control select-picker" name="project_id" id=""
                    ...
                    <div class="col-lg-4 col-md-6">
                        <x-forms.label class="mt-3" fieldId="category_id"
                            :fieldLabel="__('modules.expenses.expenseCategory')" fieldRequired="true">
                        </x-forms.label>
                        <x-forms.input-group>
                    ...
                    <div class="col-md-6 col-lg-4">
                        <x-forms.number class="mr-0 mr-lg-2 mr-md-2" :fieldLabel="__('app.bill_no')" fieldName="bill_no"
                             fieldId="bill_no" :fieldPlaceholder="__('placeholders.bill_no')"
                            :fieldValue="$expense->bill_no" />
                    </div>

                    <div class="col-md-6 col-lg-4">
                        <x-forms.number class="mr-0 mr-lg-2 mr-md-2" :fieldLabel="__('app.challan_no')" fieldName="challan_no"
                             fieldId="challan_no" :fieldPlaceholder="__('placeholders.challan_no')"
                            :fieldValue="$expense->challan_no" />
                    </div>
...
<script>
            ...
            $(document).ready(function () {
                            ...
                            $('.voucher_no').each(function () {
                                ...
                                $(this).find('.row, .p-20, .col-md-4, .my-3')
                                       .removeClass('row p-20 col-md-4 my-3');

                            });
                            
                            const $input = $("#custom_fields_data\\[voucher-no-1_1\\]");
                            const fixedLength = 6;
                        
                            // initial fixed value
                            const fixedPart = $input.val().substring(0, fixedLength);
                        
                            // keydown event
                            $input.on("keydown", function (e) {
                                const cursorPos = this.selectionStart;
                        
                                // Backspace / Delete prevent for first 6 chars
                                if ((e.key === "Backspace" || e.key === "Delete") && cursorPos <= fixedLength) {
                                    e.preventDefault();
                                }
                            ...
                            // input event (paste / manual edit handle)
                            $input.on("input", function () {
                                if (!$(this).val().startsWith(fixedPart)) {
                                    $(this).val(fixedPart + $(this).val().substring(fixedLength));
                                }
                            });
                        
                            // cursor always after fixed part
                            $input.on("focus click", function () {
                                if (this.selectionStart < fixedLength) {
                                    this.setSelectionRange(fixedLength, fixedLength);
                                }
                            });
