...
<form id="filter-form">
    <x-filters.filter-box>
        ...
        <div class="select-box d-flex px-0 border-right-grey border-right-grey-sm-0 align-items-center" >
            <p class="mb-0 pr-2 f-14 text-dark-grey d-flex align-items-center">Section</p>
            <div class="select-filter-cotractor_type">
                <select class="form-control select-picker" name="filter_section" id="filter_section" data-container="body">
                    <option selected value="all">@lang('app.all')</option>
...
@section('content')
    ...
    <div class="content-wrapper">
        <x-forms.link-primary :link="route('steps.create')"  icon="plus">
                        Add Step
...
@push('scripts')
    ...
    <script>
        ...
        $('#filter_section').selectpicker();

        const showTable = () => {
            window.LaravelDataTables["step-table"].draw();
        };

        $('#step-table').on('preXhr.dt', function (e, settings, data) {
            data.searchText = $('#search-text-field').val();
            data.status = $('#filter_status').val();
            data.project = $('#filter_project').val();
            data.section = $('#filter_section').val();
        });
        ...
        $('#reset-filters').on('click', function () {
            ...
            $('#filter_section').val('all').selectpicker('refresh');
        ...
        $(".toggle-status").change(function () {
            var id = $(this).data("id");
            $.easyAjax({
                url: "/account/steps/" + id + "/toggle-status",
                type: "POST",
