    ...
    <x-form class="c-inv-form" id="saveOrderForm">
        ...
        <div class="row px-lg-4 px-md-4 px-3 py-3">
            ...
            <div class="col-md-6 col-lg-6 mt-3">
                ...
                <div class="form-group c-inv-select mb-4">
                    <x-forms.label fieldId="estimate_id" :fieldLabel="__('modules.estimates.estimate')" fieldRequired="true">
                    </x-forms.label>

                    <div class="select-others height-35 rounded">
                        
                            <select class="form-control select-picker" name="estimate_id" id="estimate_id" data-live-search="true">
                                <option value="">--</option>
                                @if (!is_null($estimate))
                                    @foreach($estimate as $estimate)
                                        ...
                                        @if($client && $client->projects->count() > 0)
                                            @foreach($client->projects as $project)
                                                <option value="{{ $estimate->id }}">
                                                    {{ $estimate->estimate_number }} — {{ $project->project_name }} — {{ $client->name }}
                                                </option>
                                            @endforeach
                                        @else
                                            <option disabled>
                                                {{ $estimate->name }} — {{ $client->name ?? 'No Client' }} — No Projects
                                            </option>
                                        @endif
                                    @endforeach
                                @endif
            ...
            <div class="col-md-6 col-lg-3 mt-3">
                <x-forms.label fieldId="exchange_rate" :fieldLabel="__('modules.estimates.total_estimate')">
                </x-forms.label>
                <input type="number" id="total_estimate" 
                class="px-6 position-relative text-dark font-weight-normal form-control height-35 rounded p-0 text-left f-15" value="{{ $companyCurrency->total_estimate}}" readonly>
            </div>
            <div class="col-md-6 col-lg-3 mt-3">
                <x-forms.label fieldId="total_used" :fieldLabel="__('modules.estimates.total_used')">
                </x-forms.label>
                <input type="number" id="total_used" 
                class="px-6 position-relative text-dark font-weight-normal form-control height-35 rounded p-0 text-left f-15" value="{{ $companyCurrency->total_used}}" readonly>
            </div>
...
<script>
    ...
    $(document).ready(function() {
        ...
        $('#estimate_id').on('change', function(){
            ...
            $.ajax({
                url: '{{ route("get_estimate_info") }}',
                type: 'GET',
                data: { estimate_id: estimate_id },
                success: function(response) {
                    console.log(response);

                    var estimateTotal = response.estimate_info;
                    var purchaseTotal = response.purchase_total;

                   
                    $('#total_estimate').val(estimateTotal);
                    $('#total_used').val(purchaseTotal);

                   
                    if (purchaseTotal > estimateTotal) {
                        @if(user()->id != '1')
                            Swal.fire({
                                icon: 'error',
                                title: 'Warning!',
                                text: '!!Purchase amount is gretar than the Estimated amount!!',
        ...
        $('#saveOrderForm').on('keyup', '.quantity,.cost_per_item,.item_name, .discount_value', function() {
            ...
            var perItemCost = parseFloat($row.find('.cost_per_item').val()) || 0;

            var total_estimate = parseFloat($('#total_estimate').val()) || 0;
            var total_used = parseFloat($('#total_used').val()) || 0;

            if (!total_estimate || total_estimate <= 0) {
                Swal.fire({
                    title: 'Estimate Missing!',
                    text: '⚠️ Please select Estimate first.',
                    icon: 'info',
            ...
            var new_total = total_used + amount;

            if (new_total > total_estimate) {
                Swal.fire({
                    title: 'Estimate Limit Crossed!',
                    text: '⚠️ Purchase amount is greatar than estimate amount. Please contact with your Super Admin.',
                    icon: 'warning',
        ...
        $('#saveOrderForm').on('change', '.type, #discount_type, #calculate_tax', function() {
            ...
            if (new_total > total_estimate) {
                Swal.fire({
                    title: 'Estimate Limit Crossed!',
                    text: '⚠️ Purchase amount is greatar than estimate amount. Please contact with your your Super Admin.',
                    icon: 'warning',
                    showConfirmButton: false,
                    timer: 2000,
                    timerProgressBar: true,
