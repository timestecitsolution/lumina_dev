...
<div class="d-flex px-4 py-3 c-inv-desc item-row">
            ...
            <tbody>
                ...
                <tr>
                    ...
                    <td class="border-bottom-0">
                        <input type="number" min="1"
                            ...
                            data-item-id="{{ $items->id }}" placeholder="{{ $items->purchase_price }}"
                            value="{{ $items->purchase_price }}" name="cost_per_item[]">
                    </td>
                    <td class="border-bottom-0">
                ...
                <tr class="d-none d-md-table-row d-lg-table-row">
                    ...
                    <td class="border-left-0">
                        ...
                        <input type="hidden" name="order_item_image_url[]" value="{{ $items->image_url }}">
                    </td>
                </tr>
            </tbody>
        </table>
    ...
    <script>
        ...
        $(function() {
            ...
            var quantity = parseFloat($('#sortable').find('.quantity[data-item-id="{{ $items->id }}"]').val()) || 0;
            var perItemCost = parseFloat($('#sortable').find('.cost_per_item[data-item-id="{{ $items->id }}"]').val()) || 0;
            ...
            if (new_total > total_estimate) {
                Swal.fire({
                    ...
                    willClose: () => {
                        $('#sortable').find('.quantity[data-item-id="{{ $items->id }}"]').closest('.item-row').remove();
                        calculateTotal();
                    }
                });
                return;
            ...
            {
                $('#sortable').find('.amount[data-item-id="{{ $items->id }}"]').val(decimalupto2(amount));
                $('#sortable').find('.amount-html[data-item-id="{{ $items->id }}"]').html(decimalupto2(amount));
            }
