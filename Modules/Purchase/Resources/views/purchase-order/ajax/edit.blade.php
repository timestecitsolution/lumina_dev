        ...
        <div class="row px-lg-4 px-md-4 px-3 py-3">
            ...
            <div class="col-md-6 col-lg-6 mt-3">
                <div class="form-group c-inv-select mb-4">
                    ...
                    <div class="select-others height-35 rounded">
                        <select class="form-control select-picker" name="estimate_id" id="estimate_id" data-live-search="true" disabled>
                            <option value="">--</option>
                            ...
                            @if (!is_null($estimate))
                                @foreach($estimate as $est)
                                    @php
                                        $client = $est->client;
                                        $isSelected = (isset($order) && $order->estimate_id == $est->id) ? 'selected' : '';
                                    @endphp

                                    @if($client && $client->projects->count() > 0)
                                        @foreach($client->projects as $project)
                                            <option value="{{ $est->id }}" {{ $isSelected }}>
                                                {{ $est->estimate_number }} — {{ $project->project_name }} — {{ $client->name }}
                                            </option>
                                        @endforeach
                                    @else
                                        <option disabled>
                                            {{ $est->name }} — {{ $client->name ?? 'No Client' }} — No Projects
                                        </option>
                                    @endif
                                @endforeach
                            @endif
            ...
            <div class="col-md-6 col-lg-3 mt-3">
                ...
                class="px-6 position-relative text-dark font-weight-normal form-control height-35 rounded p-0 text-left f-15" value="{{ $estimate_info}}" readonly>
            </div>
            <div class="col-md-6 col-lg-3 mt-3">
                <x-forms.label fieldId="total_used" :fieldLabel="__('modules.estimates.total_used')">
                </x-forms.label>
                <input type="number" id="total_used" 
                class="px-6 position-relative text-dark font-weight-normal form-control height-35 rounded p-0 text-left f-15" value="{{ $purchase_total}}" readonly>
            </div>
