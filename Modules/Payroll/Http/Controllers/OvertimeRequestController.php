...
use App\Models\OvertimeRequestApprovals;
...
class OvertimeRequestController extends AccountBaseController
{
    ...
    public function index(OvertimeRequestDataTable $dataTable, Request $request)
    {
        ...
        $this->months = ['january', 'february', 'march', 'april', 'may', 'june', 'july', 'august', 'september', 'october', 'november', 'december','all'];
    ...
    public function getOvertimeData(Request $request)
    {
        ...
        $overtimeHours = $overtimeRequestBase->sum('hours')/9;
        ...
        $overtimeDays = round($overtimeRequestBase->sum('hours') / 9, 2);

        $result = $overtimeDays . ' ' . ($overtimeDays == 1 ? 'day' : 'days');
    ...
    public function show($id)
    {
        ...
        $this->overtime_request_approvals = OvertimeRequestApprovals::with('actionBy')->where('overtime_request_id', $id)
            ->get()
            ->keyBy('employee_id');

        $this->validation_permissions = ValidationPermission::with(['employee','designation'])
            ->orderBy('priority', 'asc')
            ->where('validation_role_id', 2) 
            ->get();
            // dd($this->validation_permissions);
        $this->user = user();
        $approvedEmployeeIds = OvertimeRequestApprovals::where('overtime_request_id', $id)
    ...
    public function action(Request $request, $id)
    {
        ...
        // get next approval
        $validation_permissions = ValidationPermission::where('validation_role_id', 2) // adjust role
        ...
        if ($isSuperAdmin || ($this->nextApproval && $this->nextApproval->employee_id == $user->id)) {
            ...
            OvertimeRequestApprovals::create([
            ...
            if($request->action == 'rejected'){
                OvertimeRequestApprovals::where('overtime_request_id', $overtime->id)->delete();
                $overtime->update(['status'=>'rejected']);
                return back()->with('success', 'Action performed successfully');
            } else {
                // check if all approvals done
                $remaining = $validation_permissions->filter(function($perm) use ($overtime) {
        ...
        abort(403, 'You cannot approve this Overtime Request yet.');
    }
