...
use App\DataTables\LeadNoteReportDataTable;
use App\DataTables\LeadFollowReportDataTable;
...
class LeadReportController extends AccountBaseController
{
    ...
    {
        $this->pageTitle = 'modules.leadContact.visitHistory';
        ...
        if (!request()->ajax()) {
            ...
            $this->sales_man = User::join('leads', 'leads.lead_owner', '=', 'users.id')
        ...
        $tab = request('tab');
        $this->activeTab = $tab ?: 'visit';


        $dataTable = new LeadNoteReportDataTable();


        return $dataTable->render('reports.lead.visited_history', $this->data);
    }

    public function followHistory()
    {
        $this->pageTitle = 'modules.leadContact.followHistory';
        ...
        $tab = request('tab');
        $this->activeTab = $tab ?: 'follow';

        $dataTable = new LeadFollowReportDataTable();

        return $dataTable->render('reports.lead.follow_history', $this->data);
    }
