...
class ExpenseController extends AccountBaseController
{
    ...
    public function show($id)
    {
        ...
        $selectedEmployee = json_decode($this->expense->user_id, true) ?? [];
       
        $allEmployee = User::whereIn('id', $selectedEmployee ?? [])->get();
        $this->employeeNames = $allEmployee->map(fn($p) => $p->name ?? $p->name)
                                    ->implode(', ');
    ...
    public function store(StoreExpense $request)
    {
        ...
        if ($userRole && isset($userRole[0]) && $userRole[0] == 'admin') {
        ...
        $customData = $request->custom_fields_data;  
        $customData['voucher_no_1'] = mt_rand(100000, 999999) . $customData['voucher_no_1'];

        if ($customData) {
            $expense->updateCustomFieldData($customData);
        }
    ...
    public function edit($id)
    {
        ...
        $this->selectedEmployee = json_decode($this->expense->user_id, true) ?? [];
    ...
    public function update(StoreExpense $request, $id)
    {
        ...
        $customData = $request->custom_fields_data;  
        $customData['voucher_no_1'] = $customData['voucher_no_1'];
