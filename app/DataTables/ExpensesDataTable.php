...
class ExpensesDataTable extends BaseDataTable
{
    ...
    public function dataTable($query)
    {
        ...
        // $datatables->editColumn('purchase_date', fn($row) => $row->purchase_date);
    ...
    public function query()
    {
        ...
        //     'expenses.id',
        //     'expenses.project_id',
        //     'custom_fields_data.value as voucher_no',
        //     'expenses.item_name',
        //     'expenses.user_id',
        //     'expenses.price',
        //     'users.salutation',
        ...
        //     'expenses.expenses_recurring_id',
        ...
        // ->leftJoin('custom_fields_data', function ($join) {
        //     $join->on('custom_fields_data.model_id', '=', 'expenses.id')
        //          ->where('custom_fields_data.custom_field_id', 1);
        // });
        ...
        ->select(
            ...
            DB::raw('GROUP_CONCAT(DISTINCT users.name ORDER BY users.name SEPARATOR ", ") as emp_name'),
        ...
        ->leftJoin('users', function ($join) {
            $join->whereRaw(
                'JSON_CONTAINS(expenses.user_id, CONCAT(\'"\', users.id, \'"\'))'
            );
        })
        ...
        if ($request->searchText != '') {
            $model->where(function ($query) {
                ...
                $query->where('expenses.item_name', 'like', '%' . $safeTerm . '%')
                    ...
                    ->orWhere('expenses.purchase_date', 'like', '%' . $safeTerm . '%')
                    ->orWhere('custom_fields_data.value', 'like', '%' . $safeTerm . '%');
            });
        }
    ...
    protected function getColumns()
    {
        $data = [
            ...
            __('modules.expenses.voucher_no') => ['data' => 'voucher_no', 'name' => 'voucher_no', 'title' => __('modules.expenses.voucher_no')],
            ...
            __('app.menu.employees') => ['data' => 'emp_name', 'name' => 'user_id', 'exportable' => false, 'title' => __('app.menu.employees')],
            __('app.employee') => ['data' => 'emp_name', 'name' => 'user_id', 'visible' => false, 'title' => __('app.employee')],
